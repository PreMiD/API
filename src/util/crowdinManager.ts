...
export async function initialize() {
	if (!config.crowdinBase || !config.webhookUri)
		return console.warn(
			"[WARN] Some config options are not set correctly. Please check your environment variables and make sure 'CROWDINBASE' and 'CROWDINWEBHOOK' exist."
		);
				...
				oldData.documentation[key].files?.forEach(
					(file: { file: string; phrases: number }) => {
						if (
							...
							changes.totalNewStrings +=
								newData.documentation[key].files.find(f => f.file == file.file)
									?.phrases - file.phrases;
						}
					}
				);
